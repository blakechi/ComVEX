...
class CoAtNetRelativeAttention(MultiheadAttention):
    ...
    def forward(self, x: torch.tensor) -> torch.tensor:
        ...
        if H == self.pre_height and W == self.pre_width:
            ...
            relative_bias = nn.functional.interpolate(self.relative_bias, size=(H, W), mode="bilinear")
    ...
    def _update_relative_bias_and_indices(self, height: int, width: int) -> None:
        ...
        self.relative_bias = nn.functional.interpolate(
            self.relative_bias,
            size=(height, width),
            mode="bilinear"
        )
...
class CoAtNetBackbone(nn.Module):
    def __init__(
        ...
        assert (
            len(num_blocks_in_layers) == 4
        ), name_with_msg(self, "The length of `num_blocks_in_layers` must be 4")
        assert (
        ...
        if isinstance(num_channels_in_layers, list):
            assert (
                len(num_channels_in_layers) == 4
            ), name_with_msg(self, "The length of `num_channels_in_layers` must be 4")
        else:
            begin_channel = int(num_channels_in_layers)
            num_channels_in_layers = [begin_channel // (2**layer_idx) for layer_idx in range(1, 5)]
        ...
        height_in_layers = [image_height / (2**layer_idx) for layer_idx in range(2, 6)]
        width_in_layers = [image_width / (2**layer_idx) for layer_idx in range(2, 6)]
        ...
        self.s_1 = self._build_layer("s_1",                                                   # layer name
            height_in_layers[0], width_in_layers[0],                                          # input size
            num_channels_in_layers[0], num_channels_in_layers[1], expand_scale_in_layers[0],  # dimension-related
            num_blocks_in_layers[0], block_type_in_layers[0]                                  # block-related
            **kwargs
        )
        self.s_2 = self._build_layer("s_2",                                                   # layer name
            ...
            num_blocks_in_layers[1], block_type_in_layers[1]                                  # block-related
            **kwargs
        )
        self.s_3 = self._build_layer("s_3",                                                   # layer name
            ...
            num_blocks_in_layers[2], block_type_in_layers[2]                                  # block-related
            **kwargs
        )
        self.s_4 = self._build_layer("s_4",                                                   # layer name
            height_in_layers[0], width_in_layers[0],                                          # input size
            num_channels_in_layers[3], num_channels_in_layers[3], expand_scale_in_layers[3],  # dimension-related
            num_blocks_in_layers[3], block_type_in_layers[3]                                  # block-related
            **kwargs
        )
