...
from os import name
from typing import Literal, Optional, Union, Tuple Dict, NamedTuple
...
class BiFPNIntermediateNode(BiFPNNode):
    ...
    def forward(self, x_main: torch.Tensor, x_sub: torch.Tensor) -> torch.Tensor:
        x_sub = self.resize(x_sub)
        x_stack = torch.stack([x_main, x_sub], dim=0)

        return self.fuse_features(x_stack, self.weights)
...
class BiFPNOutputNode(BiFPNNode):
    ...
    def forward(self, x_identical: torch.Tensor, x_main: torch.Tensor, x_sub: torch.Tensor) -> torch.Tensor:
        x_sub = self.resize(x_sub)
        x_stack = torch.stack([x_identical, x_main, x_sub], dim=0)
